                ...
                <div class="mt-6 space-y-4">
                    @foreach($steps as $key => $label)
                        ...
                        <div class="border rounded-xl p-4">
                            ...
                            @if($document?->path)
                                <a href="{{ Storage::url($document->path) }}" target="_blank" class="inline-flex mt-3 text-sm text-red-800 hover:underline">Open document</a>
                            @endif
                ...
                <form method="POST" action="{{ route('landlord.verification.update') }}" enctype="multipart/form-data" class="space-y-6 mt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="company_name" value="{{ old('company_name', $landlord->company_name) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Phone</label>
                            <input type="text" name="phone" value="{{ old('phone', $landlord->phone) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Company Registration Number</label>
                            <input type="text" name="company_registration_number" value="{{ old('company_registration_number', $landlord->company_registration_number) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tax Identification Number</label>
                            <input type="text" name="tax_identification_number" value="{{ old('tax_identification_number', $landlord->tax_identification_number) }}" class="w-full border border-gray-300 rounded-lg px-4 py-3" required>
                        </div>
                    </div>
                    ...
                    <button type="submit" class="bg-red-800 text-white px-6 py-3 rounded-lg font-semibold hover:bg-red-900 transition">
